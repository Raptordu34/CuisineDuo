...
export default function EditRecipeModal({ recipe, onClose, onSave, onDelete, tasteParams: initialTasteParams }) {
  const { t } = useLanguage()
  ...
  const [equipment, setEquipment] = useState(
    (recipe.equipment || []).map((e) => ({ name: e.name || '' }))
  )
  ...
  const [tips, setTips] = useState(
    (recipe.tips || []).map((tip) => ({ text: tip.text || '' }))
  )
  const [tasteParams, setTasteParams] = useState({
    sweetness: initialTasteParams?.sweetness || null,
    saltiness: initialTasteParams?.saltiness || null,
    spiciness: initialTasteParams?.spiciness || null,
    acidity: initialTasteParams?.acidity || null,
    bitterness: initialTasteParams?.bitterness || null,
    umami: initialTasteParams?.umami || null,
    richness: initialTasteParams?.richness || null,
  })
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/40 pb-16 md:pb-0" onClick={onClose}>
      ...
      >
        <div className="p-4 border-b border-gray-200 sticky top-0 bg-white z-10 space-y-2 shrink-0 sm:rounded-t-2xl">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              <h2 className="text-lg font-bold text-gray-900">{t('recipes.editRecipe')}</h2>
        ...
        <div className="shrink-0 p-4 border-t border-gray-200 bg-white space-y-2 sm:rounded-b-2xl">
          ...
          <button
            ...
            className={`w-full px-4 py-2.5 rounded-full text-sm font-medium transition-colors cursor-pointer ${
              confirmDelete
                ? 'bg-red-500 hover:bg-red-600 text-white'
                : 'border border-red-300 text-red-500 hover:bg-red-50'
            }`}
          >
            {confirmDelete ? t('recipes.deleteConfirm') : t('recipes.delete')}
          </button>
        </div>
      </div>
